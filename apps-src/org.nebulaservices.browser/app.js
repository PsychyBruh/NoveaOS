class Main {
    ...
    initContextMenus() {
        if (parent.novea?.contextMenu) {
            parent.novea.contextMenu.attach(this.bookmarksBar, {
                root: [
                    {
                        title: 'Add Bookmark',
    ...
    getDisplayUrl(url) {
        if (!url) return '';
        if (parent.novea?.net?.decodeUrl && url.includes(location.origin)) {
            try {
                return parent.novea.net.decodeUrl(url);
            } catch (e) {
                return url;
            }
        }
    ...
    switchToTab(tabId) {
        ...
        if (activeTab.url) {
            if (!activeTab.iframe) {
                ...
                activeTab.iframe.src = parent.novea?.net?.encodeUrl(this.getDisplayUrl(activeTab.url)) || activeTab.url;
            }
    ...
    async navigateToUrl(url) {
        ...
        const encodedUrl = parent.novea?.net?.encodeUrl(url) || url;
    ...
    toggleBookmarksBar(visible = null) {
        ...
        if (parent.novea?.settings) {
            parent.novea.settings.set('bookmarks-visible', isVisible);
        }
    ...
    addBookmark() {
        ...
        if (parent.novea?.dialog) {
            parent.novea.dialog.prompt({
                title: 'Add Bookmark',
        ...
        this.bookmarks.forEach((bookmark, index) => {
            ...
            item.addEventListener('click', () => {
                this.navigateToUrl(bookmark.url);
                    ...
                    if (parent.novea?.contextMenu) {
            parent.novea.contextMenu.attach(item, {
                    root: [
                        {
                            title: 'Edit',
    ...
    editBookmark(index) {
        ...
        if (!bookmark || !parent.novea?.dialog) return;
    ...
    loadBookmarks() {
        try {
            return parent.novea.settings.get('browser-bookmarks') || '[]';
        } catch {
            return [];
        }
    }
    ...
    saveBookmarks() {
        parent.novea.settings.set('browser-bookmarks', this.bookmarks);
    }
...
function firstTime() {
            if (!parent.novea.settings.get('browser-fv')) {
        parent.novea.dialog.alert({
            title: 'NoveaOS',
            icon: '/assets/logo.svg',
            body: 'Welcome to NoveaOSes browser! This app is work-in-progress but it should behave just fine :D Also, there are keybinds! Just use alt instead of ctrl'
        });

        parent.novea.settings.set('browser-fv', true);
    }
}

(async () => {
    const mod = await parent.novea.packages.import('novea.kv');
