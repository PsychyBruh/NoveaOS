class FirewallApp {
    ...
    async init() {
        ...
        this.p2pClient = parent.novea.p2p;
    ...
    updatePorts() {
        if (!parent.novea?.net?.loopback?.resolver) return;

        const currentPorts = Object.keys(parent.novea.net.loopback.resolver).map(Number);
